  ...
  <section v-if="personCredits" class="person-filmo">
    <h1>
      ...
      <el-button
        type="primary"
        icon="el-icon-plus"
        @click="addFilmToFilmo"
        class="add-filmo-cast"
        plain
      >Ajouter un film
      </el-button>
    </h1>
        ...
        <div class="title-and-date">
          <div class="original-title">
            ...
            <el-autocomplete
              ...
              :fetch-suggestions="querySearchMovie"
              placeholder="Titre"
              @select="handleSelectMovie(movie, $event)"
            >
            ...
            <div class="jlist">
              <div v-for="(character, index) in movie.character" :key="index" class="jitem">
                ...
                <el-button
                  type="primary"
                  icon="el-icon-delete"
                  @click="deleteCharac(movie, character)"
                  plain
                ></el-button>
              </div>
              <el-button
...
export default {
  ...
  methods: {
    ...
    deleteJob(movie, job) {
      ...
      .then(() => {
        this.$store.commit('REMOVE_JOB_FROM_MOVIEFILMO', { movie, job })
      })
      .catch(() => {
      });
    },
    addCharac(movie) {
      this.$store.commit('ADD_CHARAC_TO_MOVIEFILMO', { movie, character: { name: null } })
    },
    deleteCharac(movie, character) {
      ...
      .then(() => {
        this.$store.commit('REMOVE_CHARAC_FROM_MOVIEFILMO', { movie, character })
      })
      .catch(() => {
      });
    },
