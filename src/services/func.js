export default {
  ...
  genJobsFiltersList(persons) {
    let JobsList = []
    persons.forEach(p => {
      p.person.known_for_department.forEach(pj => {
        if(!JobsList.find(j=>j.name.includes(pj.name.substring(0, 3)))) {
          switch (pj.name.substring(0, 3)) {
            case 'Act':
              JobsList.push({name:'Acteurs', order: 1});
              break;
            case 'Réa':
              JobsList.push({name:'Réalisateurs', order: 2});
              break;
            case 'Scé':
              JobsList.push({name:'Scénaristes', order: 3});
              break;
            case 'Mus':
              JobsList.push({name:'Musiciens', order: 4});
              break;
            case 'Pho':
              JobsList.push({name:'Photographes', order: 5});
              break;
            case 'Déc':
              JobsList.push({name:'Décorateurs', order: 6});
              break;
            case 'Pro':
              JobsList.push({name:'Producteurs', order: 7});
              break;
          }
        }
      })  
